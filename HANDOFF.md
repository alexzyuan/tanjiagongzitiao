...
## 1. 当前任务目标
完成一个钉钉企业内部工资条应用：HR 导入 Excel 后匹配企业通讯录、配置员工工资条并通过钉钉工作通知发送。管理员需要管理工资条、权限、模板、存证和报表；员工仅能在钉钉移动端查看自己的工资条。当前新增方向是使用钉钉互动卡片替代普通工作通知链接，以获得更大的卡片呈现和可变量标题。互动卡片模板已在钉钉卡片平台创建，但尚未接入应用发送链路。

## 2. 当前项目状态
- 已完成：管理端、三步 Excel 导入、SQLite 加密存储、企业通讯录匹配、钉钉免登、钉钉工作通知 `link` 投递、单独发送、员工端页面、权限与模板配置基础能力。
- 正在处理：钉钉互动卡片模板配置及后端发送集成的技术路径。
- 还没开始：将互动卡片 API 接入工资条投递、待办自动创建、可靠的远程撤回、互动卡片的逐员工私有数据发送与更新。
...
- 在钉钉卡片平台创建模板 `c95d8b9a-05c6-4aeb-8821-25558ecb42dc.schema`，名称为 `te`。
- 在该互动卡片模板中创建公开普通文本变量 `salary_period_title`，用途为工资所属年月标题。
- 上传工资条互动卡片图片到钉钉媒体库，得到 `media_id`：`@lALPD1PWVmC9GlPNBD7NBag`。
...
- 当前开发分支是 `codex/salary-slip-internal-app`；大规模或实验性改动必须新建 `codex/` 前缀分支。
...
## 6. 已验证结果
- `4c30338 fix: restore supported DingTalk link notification` 是当前分支最新提交。
...
## 9. 下一步最小动作
先在钉钉卡片平台打开模板 `c95d8b9a-05c6-4aeb-8821-25558ecb42dc.schema`，选中新增的“基础文本”组件，在右侧“内容”变量选择控件中选择 `salary_period_title`，设置 Mock 值为“2026年07月工资条”，保存但不要发布或发送真实消息，除非用户明确授权。

## 10. 给新对话的启动提示词
请先阅读项目根目录的 `HANDOFF.md`，用简短清单复述你对当前工资条项目状态、已完成事项、互动卡片边界和下一步最小动作的理解。不要立刻修改代码、发布卡片、发送通知或改生产配置；等待我确认后再执行。
