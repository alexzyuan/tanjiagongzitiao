...
describe("salary delivery", () => {
  ...
  it("sends one selected employee through the same audited delivery path", async () => {
    ...
    const auth = await app.inject({ method: "POST", url: "/v1/auth/dev" });
    const cookie = auth.headers["set-cookie"]?.split(";")[0];
    const draft = await app.inject({ method: "POST", url: "/v1/salary-batches", headers: { cookie }, payload: { payrollMonth: "2026-08", title: "2026年08月工资条", rows: [{ userId: "employee-a", name: "员工A", 实发金额: 10000 }, { userId: "employee-b", name: "员工B", 实发金额: 9000 }] } });
    ...
    const send = await app.inject({ method: "POST", url: `/v1/salary-batches/${batch.id}/items/${itemId}/send`, headers: { cookie }, payload: {} });
    expect(send.statusCode).toBe(200);
    expect(store.listDeliveries(batch.id)).toHaveLength(1);
    expect(store.listDeliveries(batch.id)[0]?.employeeUserId).toBe("employee-a");
    const refreshed = await app.inject({ method: "GET", url: `/v1/salary-batches/${batch.id}`, headers: { cookie } });
    expect(refreshed.json().items[0].deliveryStatus).toBe("delivered");
    expect(refreshed.json().items[1].deliveryStatus).toBeUndefined();
    await app.close();
  });
  ...
  it("persists a reusable salary slip template", async () => {
    ...
    const auth = await app.inject({ method: "POST", url: "/v1/auth/dev" });
    const cookie = auth.headers["set-cookie"]?.split(";")[0];
    const settings = { netAmountField: "实发工资", hideEmptyFields: true, feedbackEnabled: false, confirmationEnabled: true, notice: "保密", greeting: "{name}，辛苦了", theme: "default", visibleFields: ["基本工资"], fieldGroups: [{ id: "income", name: "应发工资", fieldKeys: ["基本工资"] }] };
    const created = await app.inject({ method: "POST", url: "/v1/salary-slip-templates", headers: { cookie }, payload: { name: "常规工资条", settings } });
    expect(created.statusCode).toBe(200);
    expect((await app.inject({ method: "GET", url: "/v1/salary-slip-templates", headers: { cookie } })).json()).toMatchObject([{ name: "常规工资条", settings }]);
    await app.close();
  });
