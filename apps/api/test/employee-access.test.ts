...
describe("employee salary access", () => {
  it("only exposes the signed-in employee's own current salary slip", async () => {
    ...
    const draft = await app.inject({ method: "POST", url: "/v1/salary-batches", headers: { cookie: admin }, payload: { payrollMonth: "2026-08", title: "2026年08月工资条", rows: [{ userId: "employee-a", name: "员工A", 基本工资: 12000 }, { userId: "employee-b", name: "员工B", 基本工资: 9000 }] } });
    ...
    const list = await app.inject({ method: "GET", url: "/v1/me/salary-slips", headers: { cookie: employee } });
    expect(list.statusCode).toBe(200);
    expect(list.json()).toHaveLength(1);
    expect(list.json()[0].item.employeeUserId).toBe("employee-a");
    expect(list.json()[0].item.fields.基本工资).toBe(12000);

    const other = sessionCookie(await app.inject({ method: "POST", url: "/v1/auth/dev", payload: { userId: "employee-b", name: "员工B" } }));
    const ownDetail = await app.inject({ method: "GET", url: `/v1/me/salary-slips/${batchId}`, headers: { cookie: other } });
