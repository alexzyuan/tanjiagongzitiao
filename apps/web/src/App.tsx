...
import { api, ensureSession, type Batch, type DirectoryUser, type EmployeeMatchStrategy, type Identity, type ReportSummary, type SalaryImportPreview, type SalaryItem, type SalarySlipDisplaySettings, type SalarySlipFieldGroup, type SalarySlipTemplate, type Settings } from "./api";
...
const stateLabel: Record<string, string> = { draft: "未发送", scheduled: "已排期", sending: "发送中", sent: "已发送", partially_failed: "部分失败", withdrawn: "已撤回", archived: "已归档", viewed: "已查看", unread: "未查看", confirmed: "已确认", unconfirmed: "未确认", 在职: "在职" };
...
export function SalaryManagement({ refreshKey, onChanged }: { refreshKey: number; onChanged: () => void }) {
  const [batches, setBatches] = useState<Batch[]>([]);
  const [month, setMonth] = useState(currentMonth());
  const [monthOpen, setMonthOpen] = useState(false);
  const [query, setQuery] = useState("");
  const [statusFilter, setStatusFilter] = useState<"all" | "unread" | "unconfirmed" | "failed">("all");
  ...
  const monthBatches = useMemo(() => batches.filter(batch => batch.payrollMonth === month), [batches, month]);
  const activeBatch = monthBatches.find(batch => batch.id === activeBatchId) ?? monthBatches[0];
  const detailBatch = detailBatchId ? batches.find(batch => batch.id === detailBatchId) : undefined;
  useEffect(() => {
    setActiveBatchId(current => monthBatches.some(batch => batch.id === current) ? current : monthBatches[0]?.id);
    setSelectedItems([]);
  }, [monthBatches]);
  useEffect(() => {
    if (!activeBatch) { setDetail(undefined); return; }
    api<Batch>(`/v1/salary-batches/${activeBatch.id}`).then(setDetail).catch(reason => setError(errorText(reason)));
  }, [activeBatch?.id]);
  const employees = useMemo(() => {
    const items = detail?.items ?? [];
    const needle = query.trim().toLowerCase();
    return items.filter(item => {
      const matchesQuery = !needle || [item.employeeName, item.employeeNo, item.department, item.position].filter(Boolean).some(value => value?.toLowerCase().includes(needle));
      const matchesStatus = statusFilter === "all" || (statusFilter === "unread" && !item.viewedAt) || (statusFilter === "unconfirmed" && !item.confirmedAt) || (statusFilter === "failed" && activeBatch?.state === "partially_failed");
      return matchesQuery && matchesStatus;
    });
  }, [activeBatch?.state, detail?.items, query, statusFilter]);
  const unread = (detail?.items ?? []).filter(item => !item.viewedAt).length;
  const unconfirmed = (detail?.items ?? []).filter(item => !item.confirmedAt).length;
  ...
  async function sendActive(action: "send" | "resend" | "withdraw") {
    ...
    const label = action === "withdraw" ? "撤回" : action === "resend" ? "重新发送" : "发送";
  ...
  async function sendIndividual(batch: Batch, item: SalaryItem) {
    setBusy(true); setError(undefined);
    try { await api(`/v1/salary-batches/${batch.id}/items/${item.id}/send`, { method: "POST", body: JSON.stringify({}) }); setMessage(`${item.employeeName} 的工资条已提交发送`); await load(); onChanged(); }
    catch (reason) { setError(errorText(reason)); }
    finally { setBusy(false); }
  }
  ...
  return <section className="salary-workspace">
    <div className="salary-heading"><button className="back-button" onClick={() => window.history.back()}>‹ <span>返回</span></button><h2>{month.replace("-", "年")}月工资条 <small>{month.replace("-", "/")}</small></h2><span className="security-badge"><Icon name="shield" size={14} />敏感数据加密</span></div>
    <div className="salary-alert"><span>📣</span><strong>导入数据异常：{activeBatch?.state === "partially_failed" ? 1 : 0}</strong><button className="link-button" onClick={() => setStatusFilter("failed")}>前往处理 ›</button><span className="alert-divider" /><span>未查看：<strong>{unread}</strong></span><button className="link-button" onClick={() => setStatusFilter("unread")}>DING查看 ↗</button><span>未确认：<strong>{unconfirmed}</strong></span><button className="link-button" onClick={() => setStatusFilter("unconfirmed")}>DING确认 ↗</button><div className="manager-card"><span className="avatar blue">管</span><span>管理员：<strong>企业管理员</strong></span><button className="icon-button" onClick={() => window.dispatchEvent(new CustomEvent("salary-open-permissions"))}>›</button></div></div>
    <div className="salary-controls"><button className="button primary" onClick={() => setMode("import")}><Icon name="plus" size={17} />上传工资表</button><button className="self-send-tip" onClick={() => setMode("manual")}><strong>自己手发一条试试</strong><span>感受上传、发送全流程</span><Icon name="send" size={17} /></button><div className="month-picker-wrap"><button className="month-picker" onClick={() => setMonthOpen(value => !value)}>‹ <strong>{month.replace("-", "年")}月</strong> ›</button>{monthOpen && <div className="month-panel"><div className="month-panel-head"><button onClick={() => shiftMonth(-12)}>«</button><strong>{month.slice(0, 4)}</strong><button onClick={() => shiftMonth(12)}>»</button></div><div className="month-grid">{Array.from({ length: 12 }, (_, index) => { const candidate = `${month.slice(0, 4)}-${String(index + 1).padStart(2, "0")}`; return <button className={candidate === month ? "active" : ""} key={candidate} onClick={() => { setMonth(candidate); setMonthOpen(false); }}>{index + 1}月</button>; })}</div></div>}</div></div>
    <div className="salary-overview"><div className="overview-title"><strong>{activeBatch?.title ?? `${month.replace("-", "年")}月工资条`}</strong><span>{activeBatch ? <Status state={activeBatch.state} /> : "暂无工资表"}</span></div><div className="overview-stat"><span>已发送</span><strong>{activeBatch?.sent ?? 0}/{activeBatch?.total ?? 0}</strong></div><div className="overview-stat"><span>已查看</span><strong>{activeBatch?.viewed ?? 0}</strong></div><div className="overview-stat"><span>已确认</span><strong>{activeBatch?.confirmed ?? 0}</strong></div><div className="overview-actions"><button className="text-button" onClick={() => window.dispatchEvent(new CustomEvent("salary-open-settings"))}>设置</button><button className="text-button muted" disabled>删除</button><button className="button secondary" onClick={() => activeBatch && setDetailBatchId(activeBatch.id)}>查看发送</button></div></div>
    <div className="employee-table-card"><div className="employee-toolbar"><label className="search"><Icon name="search" size={17} /><input value={query} placeholder="搜索姓名/工号/职位" onChange={event => setQuery(event.target.value)} /></label><select className="filter-select" value={statusFilter} onChange={event => setStatusFilter(event.target.value as typeof statusFilter)}><option value="all">筛选</option><option value="unread">未查看</option><option value="unconfirmed">未确认</option><option value="failed">发送失败</option></select><div className="more-wrap"><button className="button secondary" onClick={() => setMoreOpen(value => !value)}>更多⌄</button>{moreOpen && <div className="more-menu"><button onClick={() => { setMoreOpen(false); window.dispatchEvent(new CustomEvent("salary-open-settings")); }}>设置工资条</button><button disabled={!activeBatch || activeBatch.state === "withdrawn"} onClick={() => void sendActive("withdraw")}>全部撤回</button><a href={activeBatch ? `/v1/reports/summary.csv?payrollMonth=${activeBatch.payrollMonth}` : "/v1/reports/summary.csv"} onClick={() => setMoreOpen(false)}>导出 Excel 明细</a><button onClick={printEvidence}>导出 PDF 存证</button></div>}</div><button className="button secondary" disabled>定时发送</button><button className="button primary" disabled={!activeBatch || busy} onClick={() => void sendActive(activeBatch?.state === "draft" ? "send" : "resend")}><Icon name="send" size={15} />全部发送</button></div><div className="table-scroll salary-employee-scroll"><table className="salary-employee-table"><thead><tr><th><input type="checkbox" aria-label="全选员工" checked={Boolean(employees.length && selectedItems.length === employees.length)} onChange={event => setSelectedItems(event.target.checked ? employees.map(item => item.id) : [])} /></th><th>姓名</th><th>员工状态</th><th>实发工资</th><th>发送状态</th><th>查看状态</th><th>确认状态</th><th>操作</th></tr></thead><tbody>{employees.map(item => { const net = activeBatch ? item.fields[activeBatch.displaySettings.netAmountField] : undefined; return <tr key={item.id}><td><input type="checkbox" aria-label={`选择${item.employeeName}`} checked={selectedItems.includes(item.id)} onChange={event => setSelectedItems(value => event.target.checked ? [...value, item.id] : value.filter(id => id !== item.id))} /></td><td><div className="employee-cell"><span className="avatar blue">{item.employeeName.slice(0, 1)}</span><span><strong>{item.employeeName}</strong><small>{item.department ?? item.position ?? item.employeeNo ?? "员工"}</small></span></div></td><td><Status state="在职" /></td><td className="money-cell">{typeof net === "number" ? formatSalaryValue(net) : "已加密"}</td><td><Status state={activeBatch?.state === "sent" ? "sent" : activeBatch?.state ?? "draft"} /></td><td><Status state={item.viewedAt ? "viewed" : "unread"} /></td><td><Status state={item.confirmedAt ? "confirmed" : "unconfirmed"} /></td><td><button className="text-button" disabled={!activeBatch || busy || activeBatch.state === "sent"} onClick={() => activeBatch && void sendIndividual(activeBatch, item)}>单独发送</button></td></tr>; })}</tbody></table>{!employees.length && <EmptyState label={activeBatch ? "当前筛选条件下暂无员工" : "当前月份暂无工资表"} />}</div></div>
    {detailBatch && <BatchDetail batch={detailBatch} onClose={() => setDetailBatchId(undefined)} />}
    {mode === "manual" && <ManualPanel onClose={() => setMode(undefined)} onCreated={() => { setMode(undefined); load(); onChanged(); }} />}
    {mode === "import" && <ImportWizard onClose={() => setMode(undefined)} onCreated={async batchId => { setMode(undefined); await load(); setActiveBatchId(batchId); onChanged(); }} />}
  </section>;
}
...
function BatchDetail({ batch, onClose }: { batch: Batch; onClose: () => void }) {
  ...
  return <div className="drawer-backdrop" onMouseDown={event => { if (event.currentTarget === event.target) onClose(); }}><aside className="drawer"><div className="drawer-header"><div><span className="eyebrow">工资表详情</span><h3>{batch.title}</h3></div><button className="icon-button" onClick={onClose} aria-label="关闭">×</button></div>{error && <div className="notice error">{error}</div>}<div className="detail-summary"><span><small>月份</small>{batch.payrollMonth}</span><span><small>发送进度</small>{batch.sent}/{batch.total}</span><span><small>查看确认</small>{batch.viewed}/{batch.confirmed}</span></div><div className="employee-list">{detail?.items?.map(item => <EmployeeRow item={item} batch={batch} key={item.id} />)}</div></aside></div>;
}

function EmployeeRow({ item, batch }: { item: SalaryItem; batch: Batch }) {
  const net = item.fields[batch.displaySettings.netAmountField];
  return <div className="employee-row"><span className="avatar pale">{item.employeeName.slice(0, 1)}</span><div className="employee-main"><strong>{item.employeeName}</strong><small>{item.employeeNo ?? item.employeeUserId}{item.department ? ` · ${item.department}` : ""}</small></div><span className="employee-net">{typeof net === "number" ? `¥ ${formatSalaryValue(net)}` : "已加密"}</span><Status state={item.confirmedAt ? "confirmed" : item.viewedAt ? "viewed" : "unread"} /></div>;
}
...
function ImportWizard({ onClose, onCreated }: { onClose: () => void; onCreated: (batchId: string) => Promise<void> }) {
  ...
  const [directoryQuery, setDirectoryQuery] = useState("");
  const [directoryResults, setDirectoryResults] = useState<DirectoryUser[]>([]);
  const [settings, setSettings] = useState<SalarySlipDisplaySettings>({ netAmountField: "", hideEmptyFields: true, feedbackEnabled: true, confirmationEnabled: false, notice: "工资条属于敏感信息，请注意保密", greeting: "{name}，工作辛苦啦", theme: "default", visibleFields: [], fieldGroups: [] });
  const [busy, setBusy] = useState(false);
  const [error, setError] = useState<string>();
  const [settingsMessage, setSettingsMessage] = useState<string>();
  const [templates, setTemplates] = useState<SalarySlipTemplate[]>([]);
  const unresolved = preview?.rows.filter(row => row.status !== "matched" && !resolutions[row.row]) ?? [];
  const employeeFields = useMemo(() => preview?.sourceRows.find(row => row.kind === "employee") ? Object.keys(preview.sourceRows.find(row => row.kind === "employee")!.source) : [], [preview]);
  const salaryFields = useMemo(() => employeeFields.filter(field => !/(姓名|name|工号|employee.?no|user.?id|用户.?id|部门|职位)/i.test(field)), [employeeFields]);
  useEffect(() => { if (step === "settings") api<SalarySlipTemplate[]>("/v1/salary-slip-templates").then(setTemplates).catch(reason => setError(errorText(reason))); }, [step]);
  ...
  async function previewWorkbook(event: FormEvent) {
    ...
    form.append("payrollMonth", month); form.append("title", title); form.append("matchStrategy", strategy); form.append("file", file);
    try {
      const value = await api<SalaryImportPreview>("/v1/salary-batches/import/preview", { method: "POST", body: form });
      const fields = Object.keys(value.sourceRows.find(row => row.kind === "employee")?.source ?? {}).filter(field => !/(姓名|name|工号|employee.?no|user.?id|用户.?id|部门|职位)/i.test(field));
      setPreview(value); setResolutions({}); setSettings(current => ({ ...current, netAmountField: fields.find(field => /实发/.test(field)) ?? current.netAmountField, visibleFields: fields, fieldGroups: defaultFieldGroups(fields) })); setStep("preview");
    } catch (reason) { setError(errorText(reason)); }
    finally { setBusy(false); }
  }
  ...
  async function complete() {
    ...
    setBusy(true); setError(undefined);
    try {
      const result = await api<{ batchId: string }>("/v1/salary-batches/import/commit", { method: "POST", body: JSON.stringify({ previewId: preview.previewId, resolutions: Object.entries(resolutions).map(([row, user]) => ({ row: Number(row), userId: user.userId })), displaySettings: settings }) });
  ...
  async function saveTemplate() {
    ...
    if (!name?.trim()) return;
    setBusy(true); setError(undefined);
    try { const template = await api<SalarySlipTemplate>("/v1/salary-slip-templates", { method: "POST", body: JSON.stringify({ name: name.trim(), settings }) }); setTemplates(current => [template, ...current]); setSettingsMessage(`模板“${template.name}”已保存`); }
    catch (reason) { setError(errorText(reason)); }
    finally { setBusy(false); }
  }
  ...
  function selectUser(row: number, user: DirectoryUser) {
    ...
    setActiveRow(undefined); setDirectoryQuery(""); setDirectoryResults([]);
  }

  const steps = [{ key: "upload", label: "上传Excel工资表" }, { key: "preview", label: "预览表格数据" }, { key: "settings", label: "设置工资条" }] as const;
  return <section className="import-wizard">
    <header className="import-wizard-header"><button className="back-button" onClick={onClose}>‹ <span>返回</span></button><h2>上传工资表</h2><span className="security-badge"><Icon name="shield" size={14} />敏感数据加密</span></header>
    <div className="wizard-stepper">{steps.map((item, index) => <div className={`wizard-step ${step === item.key ? "active" : ""} ${steps.findIndex(value => value.key === step) > index ? "done" : ""}`} key={item.key}><span>{steps.findIndex(value => value.key === step) > index ? "✓" : index + 1}</span><strong>{item.label}</strong></div>)}</div>
    {step === "upload" && <form className="wizard-panel wizard-upload" onSubmit={previewWorkbook}>
      <h3>导入 Excel 工资表</h3><p>上传后只创建限时预览，不会生成工资条或发送通知。</p>
      <div className="form-grid"><Field label="发薪月份"><input value={month} onChange={event => setMonth(event.target.value)} pattern="\d{4}-\d{2}" required /></Field><Field label="工资条标题"><input value={title} onChange={event => setTitle(event.target.value)} required /></Field><Field label="匹配企业人员"><select value={strategy} onChange={event => setStrategy(event.target.value as EmployeeMatchStrategy)}><option value="name">按姓名匹配</option><option value="employeeNo">按工号匹配</option><option value="userId">按钉钉用户 ID 匹配</option></select></Field><Field label="工资表文件"><input type="file" accept=".xlsx,.xls,.csv" onChange={event => setFile(event.target.files?.[0])} required /></Field></div>
      {error && <div className="notice error">{error}</div>}<div className="wizard-actions"><button type="button" className="button secondary" onClick={onClose}>取消</button><button className="button primary" disabled={busy}>{busy ? "正在读取" : "下一步"}</button></div>
    </form>}
    {step === "preview" && preview && <div className="wizard-panel">
      <div className="wizard-panel-title"><div><h3>预览表格数据</h3><p>汇总行不会导入；异常人员需匹配到企业通讯录后才能进入设置。</p></div><span className="preview-count">已匹配 {preview.matched} · 待处理 {unresolved.length}</span></div>
      <div className="source-preview-scroll"><table className="source-preview-table"><thead><tr><th>行</th><th>状态</th>{employeeFields.map(field => <th key={field}>{field}</th>)}</tr></thead><tbody>{preview.sourceRows.map(row => <tr className={row.kind === "summary" ? "ignored-summary" : ""} key={row.row}><td>{row.row}</td><td>{row.kind === "summary" ? "汇总行，不导入" : preview.rows.find(item => item.row === row.row)?.status === "matched" ? "已匹配" : "待处理"}</td>{employeeFields.map(field => <td key={field}>{formatSalaryValue(row.source[field] as string | number | null | undefined)}</td>)}</tr>)}</tbody></table></div>
      {unresolved.length > 0 && <div className="import-match-list">{unresolved.map(row => <div className="import-match-row" key={row.row}><div><strong>第 {row.row} 行</strong><span>{row.value ?? "未提供匹配字段"}</span></div>{row.candidates.length ? <div className="match-actions">{row.candidates.map(user => <button className="text-button" type="button" key={user.userId} onClick={() => selectUser(row.row, user)}>{directoryLabel(user)}</button>)}</div> : <button className="button secondary" type="button" onClick={() => { setActiveRow(row.row); setDirectoryResults([]); }}>选择人员</button>}</div>)}</div>}
      {activeRow && <div className="directory-search"><strong>为第 {activeRow} 行选择企业人员</strong><div><input autoFocus value={directoryQuery} onChange={event => setDirectoryQuery(event.target.value)} placeholder="姓名、工号或钉钉用户 ID" /><button type="button" className="button secondary" disabled={busy || !directoryQuery.trim()} onClick={() => void searchDirectory()}>搜索</button></div>{directoryResults.map(user => <button className="directory-result" type="button" key={user.userId} onClick={() => selectUser(activeRow, user)}>{directoryLabel(user)}</button>)}</div>}
      {error && <div className="notice error">{error}</div>}<div className="wizard-actions"><button className="button secondary" type="button" onClick={() => setStep("upload")}>重新上传</button><button className="button primary" type="button" disabled={unresolved.length > 0} onClick={() => setStep("settings")}>下一步</button></div>
    </div>}
    {step === "settings" && preview && <div className="wizard-panel import-settings-grid">
      <div><h3>设置工资条</h3><p>此配置随本次工资表保存，并用于员工在钉钉中查看的详情。</p>{settingsMessage && <div className="notice success">{settingsMessage}</div>}<div className="settings-form"><Field label="保存的模板" wide><div className="template-actions"><select defaultValue="" onChange={event => { const template = templates.find(item => item.id === event.target.value); if (template) setSettings(template.settings); }}><option value="">选择并应用模板</option>{templates.map(template => <option key={template.id} value={template.id}>{template.name}</option>)}</select><button type="button" className="button secondary" disabled={busy} onClick={() => void saveTemplate()}>保存为模板</button></div></Field><Field label="实发金额字段"><select value={settings.netAmountField} onChange={event => setSettings(value => ({ ...value, netAmountField: event.target.value }))}><option value="">请选择</option>{salaryFields.map(field => <option key={field} value={field}>{field}</option>)}</select></Field><Field label="显示薪资项" wide><div className="salary-field-picker">{salaryFields.map(field => <label key={field}><input type="checkbox" checked={settings.visibleFields.includes(field)} onChange={event => setSettings(value => ({ ...value, visibleFields: event.target.checked ? [...value.visibleFields, field] : value.visibleFields.filter(item => item !== field) }))} />{field}</label>)}<button type="button" className="text-button" onClick={() => { const field = window.prompt("新增薪资项名称"); if (field?.trim()) setSettings(value => ({ ...value, visibleFields: [...value.visibleFields, field.trim()] })); }}>＋ 添加薪资项</button></div></Field><Field label="分组模板" wide><div className="salary-groups">{settings.fieldGroups.map(group => <div className="salary-group" key={group.id}><b>{group.name}</b><span>{group.fieldKeys.filter(field => settings.visibleFields.includes(field)).join("、") || "暂无字段"}</span><button type="button" className="text-button" onClick={() => setSettings(value => ({ ...value, fieldGroups: value.fieldGroups.filter(item => item.id !== group.id) }))}>删除</button></div>)}<button type="button" className="button secondary" onClick={() => { const name = window.prompt("分组名称"); if (name?.trim()) setSettings(value => ({ ...value, fieldGroups: [...value.fieldGroups, { id: `group-${Date.now()}`, name: name.trim(), fieldKeys: value.visibleFields }] })); }}>＋ 新建分组</button></div></Field><Field label="温馨提示"><textarea value={settings.notice} maxLength={500} onChange={event => setSettings(value => ({ ...value, notice: event.target.value }))} /></Field><Field label="员工关怀"><input value={settings.greeting} maxLength={200} onChange={event => setSettings(value => ({ ...value, greeting: event.target.value }))} /></Field><Field label="预览主题"><select value={settings.theme} onChange={event => setSettings(value => ({ ...value, theme: event.target.value as SalarySlipDisplaySettings["theme"] }))}><option value="default">默认背景</option><option value="technology">科技创新</option><option value="night">数智未来</option><option value="gold">日进斗金</option><option value="lotus">荷包满满</option></select></Field><div className="toggle-row"><span>空值字段隐藏</span><button type="button" className={`toggle ${settings.hideEmptyFields ? "on" : ""}`} onClick={() => setSettings(value => ({ ...value, hideEmptyFields: !value.hideEmptyFields }))}><span /></button></div><div className="toggle-row"><span>员工反馈</span><button type="button" className={`toggle ${settings.feedbackEnabled ? "on" : ""}`} onClick={() => setSettings(value => ({ ...value, feedbackEnabled: !value.feedbackEnabled }))}><span /></button></div><div className="toggle-row"><span>确认无误</span><button type="button" className={`toggle ${settings.confirmationEnabled ? "on" : ""}`} onClick={() => setSettings(value => ({ ...value, confirmationEnabled: !value.confirmationEnabled }))}><span /></button></div></div></div>
      <SalarySlipPreview title={title} settings={settings} fields={salaryFields} sample={preview.sourceRows.find(row => row.kind === "employee")?.source ?? {}} />
      {error && <div className="notice error">{error}</div>}<div className="wizard-actions"><button className="button secondary" type="button" onClick={() => setStep("preview")}>上一步</button><button className="button primary" type="button" disabled={busy || !settings.netAmountField} onClick={() => void complete()}>{busy ? "正在创建" : "完成并进入发送管理"}</button></div>
    </div>}
  </section>;
}
...
function EmployeePage({ employeeId }: { employeeId: string | undefined }) {
  ...
  const visible = settings.visibleFields.length ? settings.visibleFields : allFields.map(([key]) => key);
  const fields = allFields.filter(([key, value]) => key !== settings.netAmountField && visible.includes(key) && (!settings.hideEmptyFields || (value !== null && value !== "" && value !== 0)));
  const fieldByKey = new Map(fields);
  const groupedKeys = new Set(settings.fieldGroups.flatMap(group => group.fieldKeys));
  const renderField = ([key, value]: [string, string | number | null]) => <div className="salary-field" key={key}><span>{key}</span><strong>{formatSalaryValue(value)}</strong></div>;
  return <div className="employee-page"><div className="employee-top"><span className="brand-mark"><Icon name="wallet" size={18} /></span><div><strong>工资条</strong><small>仅本人可见</small></div><span className="employee-security"><Icon name="shield" size={15} />加密</span></div><main className="employee-sheet"><div className="employee-title"><span className="eyebrow">{payload.batch.payrollMonth}</span><h1>{payload.batch.title}</h1><p>{identity.name} · {payload.item.employeeNo ?? payload.item.employeeUserId}</p></div><div className="net-card"><span>实发金额（元）</span><strong>{typeof netValue === "number" ? formatSalaryValue(netValue) : "--"}</strong><small>工资信息属于个人敏感数据，请妥善保管</small></div><div className="salary-fields">{settings.fieldGroups.map(group => { const groupFields = group.fieldKeys.flatMap(key => { const value = fieldByKey.get(key); return value === undefined ? [] : [[key, value] as [string, string | number | null]]; }); return groupFields.length ? <section className="salary-field-group" key={group.id}><h2>{group.name}</h2>{groupFields.map(renderField)}</section> : null; })}{fields.filter(([key]) => !groupedKeys.has(key)).map(renderField)}</div><button className={`employee-confirm ${confirmed || payload.item.confirmedAt ? "confirmed" : ""}`} onClick={confirm} disabled={Boolean(confirmed || payload.item.confirmedAt)}><Icon name="check" size={19} />{confirmed || payload.item.confirmedAt ? "已确认查看" : "确认已查看"}</button><p className="employee-footnote">本工资条通过企业内部工作通知送达，查看和确认时间将生成存证记录。</p></main></div>;
}

function SalarySlipPreview({ title, settings, fields, sample }: { title: string; settings: SalarySlipDisplaySettings; fields: string[]; sample: Record<string, unknown> }) {
  const visibleFields = fields.filter(field => settings.visibleFields.includes(field));
  const groupedKeys = new Set(settings.fieldGroups.flatMap(group => group.fieldKeys));
  const renderField = (field: string) => <div className="salary-preview-field" key={field}><span>{field}</span><strong>{formatSalaryValue(sample[field] as string | number | null | undefined)}</strong></div>;
  return <aside className={`salary-slip-preview theme-${settings.theme}`} aria-label="工资条预览">
    <strong>{title}</strong>
...
function formatMoney(value: number) { return formatSalaryValue(value, true); }
export function formatSalaryValue(value: string | number | null | undefined, grouping = false): string {
  if (value === null || value === undefined || value === "") return "-";
  if (typeof value !== "number") return String(value);
  const rounded = Math.round((Math.abs(value) + Number.EPSILON) * 100) / 100 * Math.sign(value || 1);
  return grouping ? rounded.toLocaleString("zh-CN", { minimumFractionDigits: 2, maximumFractionDigits: 2 }) : rounded.toFixed(2);
}
function defaultFieldGroups(fields: string[]): SalarySlipFieldGroup[] {
  const group = (id: string, name: string, matcher: RegExp) => ({ id, name, fieldKeys: fields.filter(field => matcher.test(field)) });
  const groups = [group("attendance", "出勤情况", /(出勤|请假|迟到|早退)/), group("income", "应发工资", /(工资|补贴|绩效|业绩|提成|应发)/), group("deduction", "代扣款项", /(扣款|个税|所得税|社保|养老|失业|医保)/)];
  const grouped = new Set(groups.flatMap(item => item.fieldKeys));
  const other = fields.filter(field => !grouped.has(field));
  return other.length ? [...groups, { id: "other", name: "其他", fieldKeys: other }] : groups;
}
