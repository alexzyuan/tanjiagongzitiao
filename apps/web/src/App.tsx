...
function SalaryManagement({ refreshKey, onChanged }: { refreshKey: number; onChanged: () => void }) {
  ...
  const activeBatch = selected && monthBatches.some(batch => batch.id === selected.id) ? selected : monthBatches[0];
  useEffect(() => {
    setSelected(activeBatch);
    setSelectedItems([]);
  }, [activeBatch?.id]);
  useEffect(() => {
