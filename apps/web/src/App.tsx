...
import { api, ensureSession, type Batch, type DirectoryUser, type EmployeeMatchStrategy, type Identity, type ReportSummary, type SalaryImportPreview, type SalaryItem, type SalarySlipDisplaySettings, type Settings } from "./api";
...
function ImportWizard({ onClose, onCreated }: { onClose: () => void; onCreated: (batchId: string) => Promise<void> }) {
  ...
  const [directoryQuery, setDirectoryQuery] = useState("");
  const [directoryResults, setDirectoryResults] = useState<DirectoryUser[]>([]);
  const [settings, setSettings] = useState<SalarySlipDisplaySettings>({ netAmountField: "", hideEmptyFields: true, feedbackEnabled: true, confirmationEnabled: false, notice: "工资条属于敏感信息，请注意保密", greeting: "{name}，工作辛苦啦", theme: "default" });
  const [busy, setBusy] = useState(false);
  const [error, setError] = useState<string>();
  const unresolved = preview?.rows.filter(row => row.status !== "matched" && !resolutions[row.row]) ?? [];
  ...
  async function previewWorkbook(event: FormEvent) {
    ...
    try {
      ...
      setPreview(value); setResolutions({}); setSettings(current => ({ ...current, netAmountField: Object.keys(value.sourceRows.find(row => row.kind === "employee")?.source ?? {}).find(field => /实发/.test(field)) ?? current.netAmountField })); setStep("preview");
    } catch (reason) { setError(errorText(reason)); }
    finally { setBusy(false); }
  }
  ...
  return <section className="import-wizard">
    ...
    {step === "settings" && preview && <div className="wizard-panel import-settings-grid">
      <div><h3>设置工资条</h3><p>此配置随本次工资表保存，并用于员工在钉钉中查看的详情。</p><div className="settings-form"><Field label="实发金额字段"><select value={settings.netAmountField} onChange={event => setSettings(value => ({ ...value, netAmountField: event.target.value }))}><option value="">请选择</option>{salaryFields.map(field => <option key={field} value={field}>{field}</option>)}</select></Field><Field label="温馨提示"><textarea value={settings.notice} maxLength={500} onChange={event => setSettings(value => ({ ...value, notice: event.target.value }))} /></Field><Field label="员工关怀"><input value={settings.greeting} maxLength={200} onChange={event => setSettings(value => ({ ...value, greeting: event.target.value }))} /></Field><Field label="预览主题"><select value={settings.theme} onChange={event => setSettings(value => ({ ...value, theme: event.target.value as SalarySlipDisplaySettings["theme"] }))}><option value="default">默认背景</option><option value="technology">科技创新</option><option value="night">数智未来</option><option value="gold">日进斗金</option><option value="lotus">荷包满满</option></select></Field><div className="toggle-row"><span>空值字段隐藏</span><button type="button" className={`toggle ${settings.hideEmptyFields ? "on" : ""}`} onClick={() => setSettings(value => ({ ...value, hideEmptyFields: !value.hideEmptyFields }))}><span /></button></div><div className="toggle-row"><span>员工反馈</span><button type="button" className={`toggle ${settings.feedbackEnabled ? "on" : ""}`} onClick={() => setSettings(value => ({ ...value, feedbackEnabled: !value.feedbackEnabled }))}><span /></button></div><div className="toggle-row"><span>确认无误</span><button type="button" className={`toggle ${settings.confirmationEnabled ? "on" : ""}`} onClick={() => setSettings(value => ({ ...value, confirmationEnabled: !value.confirmationEnabled }))}><span /></button></div></div></div>
      <div className={`salary-slip-preview theme-${settings.theme}`}><strong>{title}</strong><span>{settings.greeting.replace("{name}", "员工")}</span><b>******</b><small>{settings.netAmountField || "实发金额"}</small>{settings.notice && <p><strong>温馨提示</strong>{settings.notice}</p>}<div>工资字段将在员工查看时按权限解密展示</div></div>
