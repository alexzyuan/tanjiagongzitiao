...
function PermissionCenter({ refreshKey, onChanged }: { refreshKey: number; onChanged: () => void }) {
  ...
  const [message, setMessage] = useState<string>();
  const [error, setError] = useState<string>();
  useEffect(() => { api<Batch[]>("/v1/salary-batches").then(value => { setBatches(value); setSelectedId(value[0]?.id ?? ""); }).catch(reason => setError(errorText(reason))); }, [refreshKey]);
