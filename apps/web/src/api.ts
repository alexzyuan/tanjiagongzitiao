...
export async function ensureSession(employeeId?: string): Promise<Identity> {
  if (employeeId) {
    return api<Identity>("/v1/auth/dev", {
      method: "POST",
      body: JSON.stringify({ userId: employeeId, name: employeeId }),
    });
  }
  try {
    ...
    return api<Identity>("/v1/auth/dingtalk", {
      method: "POST",
...
async function requestDingTalkAuthCode(
  ...
  if (!requestAuthCode)
    throw new Error(
      "dingtalk_jsapi_request_auth_code_unavailable_open_this_page_inside_dingtalk",
    );
  return new Promise<string>((resolve, reject) => {
    ...
    const fail = (reason: unknown) =>
      finish(reject, toError(reason, "dingtalk_auth_code_request_failed"));
    let result: unknown;
    try {
      ...
      result.then((response) => {
        const code =
          response && typeof response === "object"
            ? extractDingTalkAuthCode(response as DingTalkAuthResponse)
            : "";
        if (!code) return fail("dingtalk_auth_code_missing");
        finish(resolve, code);
      }, fail);
...
export function extractDingTalkAuthCode(response: DingTalkAuthResponse): string {
  if (typeof response.authCode === "string" && response.authCode.trim())
    return response.authCode.trim();
  if (
    typeof response.code === "string" &&
    response.code.trim() &&
    response.code.trim() !== "0"
  )
