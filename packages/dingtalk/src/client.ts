...
export class HttpDingTalkClient implements DingTalkClient {
  ...
  constructor(private readonly config: HttpDingTalkConfig) {
    if (!config.clientId || !config.clientSecret)
      throw new Error("dingtalk_client_credentials_missing");
    ...
    this.legacyApiBaseUrl = trimBaseUrl(
      config.legacyApiBaseUrl ?? DEFAULT_LEGACY_API_BASE_URL,
    );
  }
  ...
  async exchangeAuthCode(code: string): Promise<DingTalkIdentity> {
    ...
    const response = await this.requestJson(
      "identity.lookup",
      `${this.legacyApiBaseUrl}/topapi/v2/user/getuserinfo?access_token=${encodeURIComponent(appToken)}`,
      {
        method: "POST",
  ...
  async sendWorkNotification(
    ...
    const response = await this.requestJson(
      ...
      `${this.legacyApiBaseUrl}/topapi/message/corpconversation/asyncsend_v2`,
      {
        method: "POST",
  ...
  async createTodo(input: TodoTask): Promise<{ todoId: string }> {
    ...
    if (!token)
      throw new Error(`dingtalk_user_token_missing_for_todo:${input.userId}`);
    const response = await this.requestJson(
      "todo.create",
      `${this.apiBaseUrl}/v1.0/todo/users/me/personalTasks`,
      {
        method: "POST",
        headers: {
  ...
  async listDirectoryUsers(): Promise<DirectoryUser[]> {
    ...
    for (const departmentId of departmentIds) {
      ...
      do {
        const response = await this.requestJson(
          "directory.department_users",
          `${this.legacyApiBaseUrl}/topapi/user/listid?access_token=${encodeURIComponent(accessToken)}`,
          {
            method: "POST",
            headers: { "content-type": "application/json" },
        ...
        const nextCursor =
          numberValue(result, "next_cursor", "nextCursor") ?? cursor + 100;
        this.trace("directory.department_users.listed", {
          departmentId,
          cursor,
  ...
  private async listDepartmentIds(accessToken: string): Promise<number[]> {
    ...
    while (queue.length) {
      ...
      const response = await this.requestJson(
        ...
        `${this.legacyApiBaseUrl}/topapi/v2/department/listsubid?access_token=${encodeURIComponent(accessToken)}`,
        {
          method: "POST",
          headers: { "content-type": "application/json" },
  ...
  private async getAppToken(): Promise<string> {
    if (this.appToken && this.appToken.expiresAt > Date.now())
    ...
    const response = await this.requestJson("app.token", url.toString(), {
      method: "GET",
    });
    ...
    this.appToken = {
      accessToken,
      expiresAt: Date.now() + Math.max(1, expireIn) * 1000 - TOKEN_SKEW_MS,
    };
    return accessToken;
  }
  ...
  private async requestJson(
    ...
    if (!response.ok) {
      const code =
        stringValue(body, "code", "errcode", "errorCode") ??
...
function assertDingTalkSuccess(value: JsonObject, operation: string): void {
  const code =
    numberValue(value, "errcode", "errorCode") ??
    stringValue(value, "errcode", "errorCode");
  if (code !== undefined && String(code) !== "0") {
    const message =
      stringValue(value, "errmsg", "message", "errorMessage") ?? String(code);
