...
import type { SalaryBatchState, SalaryItemInput } from "@salary/domain";
...
type BatchRow = {
  ...
  assigned_admin_ids: string; created_by_id: string; created_at: string; scheduled_at: string | null; archived_at: string | null;
};
type ItemRow = {
...
export class SqliteSalaryStore implements SalaryStore {
  ...
  createBatch(input: { payrollMonth: string; title: string; createdById: string; items: SalaryItemInput[] }): StoredBatch {
    ...
    this.db.transaction(() => {
      this.db.prepare("INSERT INTO salary_batches (id, payroll_month, title, state, total, sent, viewed, confirmed, assigned_admin_ids, created_by_id, created_at) VALUES (?, ?, ?, 'draft', ?, 0, 0, 0, '[]', ?, ?)")
        .run(id, input.payrollMonth, input.title, input.items.length, input.createdById, createdAt);
  ...
  private toBatch(row: BatchRow): StoredBatch { return { id: row.id, payrollMonth: row.payroll_month, title: row.title, state: row.state, total: row.total, sent: row.sent, viewed: row.viewed, confirmed: row.confirmed, assignedAdminIds: this.adminIds(row), createdById: row.created_by_id, createdAt: row.created_at, ...(row.scheduled_at ? { scheduledAt: row.scheduled_at } : {}), ...(row.archived_at ? { archivedAt: row.archived_at } : {}), items: (this.db.prepare("SELECT * FROM salary_items WHERE batch_id = ? ORDER BY id").all(row.id) as ItemRow[]).map(item => this.toItem(item)) }; }
