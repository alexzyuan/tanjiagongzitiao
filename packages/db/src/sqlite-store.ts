...
type ItemRow = {
  ...
  fields_ciphertext: Buffer;
  fields_iv: Buffer;
  fields_auth_tag: Buffer;
...
export class SqliteSalaryStore implements SalaryStore {
  ...
  createBatch(input: {
    ...
    this.db.transaction(() => {
      this.db
        .prepare(
          "INSERT INTO salary_batches (id, payroll_month, title, state, total, sent, viewed, confirmed, assigned_admin_ids, created_by_id, display_settings, created_at) VALUES (?, ?, ?, 'draft', ?, 0, 0, 0, '[]', ?, ?, ?)",
        )
        .run(
          id,
          input.payrollMonth,
      ...
      const insert = this.db.prepare(
        "INSERT INTO salary_items (id, batch_id, employee_user_id, employee_name, employee_no, department, position, fields_ciphertext, fields_iv, fields_auth_tag) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)",
      );
  ...
  setState(id: string, state: SalaryBatchState): StoredBatch {
    ...
    this.db
      .prepare(
        "UPDATE salary_batches SET state = ?, archived_at = CASE WHEN ? = 'archived' THEN ? ELSE archived_at END WHERE id = ?",
      )
  ...
  schedule(id: string, scheduledAt: string): StoredBatch {
    ...
    this.db
      .prepare(
        "UPDATE salary_batches SET state = 'scheduled', scheduled_at = ? WHERE id = ?",
      )
      .run(scheduledAt, id);
    return this.getBatch(id);
  }
  ...
  listScheduledDue(now = new Date()): string[] {
    return (
      this.db
        .prepare(
          "SELECT id FROM salary_batches WHERE state = 'scheduled' AND scheduled_at <= ?",
        )
  ...
  assignAdmin(id: string, userId: string): StoredBatch {
    ...
    const ids = this.adminIds(batch);
    if (!ids.includes(userId))
      this.db
        .prepare(
          "UPDATE salary_batches SET assigned_admin_ids = ? WHERE id = ?",
        )
        .run(JSON.stringify([...ids, userId]), id);
    return this.getBatch(id);
  }
  ...
  removeAdmin(id: string, userId: string): StoredBatch {
    ...
    this.db
      .prepare("UPDATE salary_batches SET assigned_admin_ids = ? WHERE id = ?")
      .run(JSON.stringify(ids.filter((candidate) => candidate !== userId)), id);
    return this.getBatch(id);
  }
  ...
  assignSubAdmin(userId: string): string[] {
    ...
    this.db
      .prepare("INSERT OR IGNORE INTO salary_sub_admins (user_id) VALUES (?)")
      .run(userId);
    return this.listSubAdmins();
  }
  removeSubAdmin(userId: string): string[] {
    this.db
      .prepare("DELETE FROM salary_sub_admins WHERE user_id = ?")
      .run(userId);
    return this.listSubAdmins();
  }
  listSubAdmins(): string[] {
    return (
      this.db
        .prepare("SELECT user_id FROM salary_sub_admins ORDER BY user_id")
  ...
  markViewed(id: string, employeeUserId: string): StoredItem {
    return this.markInteraction(id, employeeUserId, "viewed");
  }
  ...
  recordAudit(input: Omit<AuditRecord, "id" | "createdAt">): AuditRecord {
    ...
    this.db
      .prepare(
        "INSERT INTO salary_audits (id, correlation_id, actor_user_id, action, target_type, target_id, outcome, metadata, created_at) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)",
      )
      .run(
        record.id,
  ...
  listAudits(): AuditRecord[] {
    return (
      this.db
        .prepare("SELECT * FROM salary_audits ORDER BY created_at")
        .all() as any[]
    ).map((row) => ({
      id: row.id,
  ...
  recordDelivery(
    ...
    return this.recordEvent("salary_deliveries", input);
  }
  listDeliveries(batchId?: string): DeliveryRecord[] {
    return this.listEvents("salary_deliveries", batchId);
  }
  recordEvidence(
    input: Omit<PaymentEvidenceRecord, "id" | "createdAt">,
  ): PaymentEvidenceRecord {
    return this.recordEvent("salary_evidence", input);
  }
  listEvidence(batchId?: string): PaymentEvidenceRecord[] {
    return this.listEvents(
      "salary_evidence",
      batchId,
    ) as PaymentEvidenceRecord[];
  }

  archiveExpired(cutoffPayrollMonth: string): string[] {
    const eligible = this.db
      .prepare(
        "SELECT id, state FROM salary_batches WHERE payroll_month < ? AND state IN ('sent', 'partially_failed', 'withdrawn')",
      )
      .all(cutoffPayrollMonth) as Pick<BatchRow, "id" | "state">[];
    this.db.transaction(() => {
      for (const batch of eligible) {
        assertTransition(batch.state, "archived");
        this.db
          .prepare(
            "UPDATE salary_batches SET state = 'archived', archived_at = ? WHERE id = ?",
          )
  ...
  setSettings(patch: Partial<AppSettings>): AppSettings {
    ...
    this.db
      .prepare(
        "INSERT INTO salary_settings (id, value) VALUES ('default', ?) ON CONFLICT(id) DO UPDATE SET value = excluded.value",
      )
  ...
  createSalaryTemplate(input: {
    ...
    this.db
      .prepare(
        "INSERT INTO salary_slip_templates (id, name, settings, created_at) VALUES (?, ?, ?, ?)",
      )
      .run(
        template.id,
        template.name,
  ...
  listSalaryTemplates(): SalarySlipTemplate[] {
    return (
      this.db
        .prepare("SELECT * FROM salary_slip_templates ORDER BY created_at DESC")
        .all() as Array<{
        id: string;
        name: string;
        settings: string;
  ...
  private markInteraction(
    ...
    kind: "viewed" | "confirmed",
  ): StoredItem {
    const row = this.itemRow(batchId, employeeUserId);
    const column = kind === "viewed" ? "viewed_at" : "confirmed_at";
    if (!row[column])
      this.db.transaction(() => {
        this.db
          .prepare(`UPDATE salary_items SET ${column} = ? WHERE id = ?`)
          .run(new Date().toISOString(), row.id);
        this.db
          .prepare(
            `UPDATE salary_batches SET ${kind} = ${kind} + 1 WHERE id = ?`,
          )
          .run(batchId);
      })();
  ...
  private batchRow(id: string): BatchRow {
    const row = this.db
      .prepare("SELECT * FROM salary_batches WHERE id = ?")
      .get(id) as BatchRow | undefined;
    if (!row) throw new Error(`salary_batch_not_found:${id}`);
    return row;
  }
  private itemRow(batchId: string, employeeUserId: string): ItemRow {
    const row = this.db
      .prepare(
        "SELECT * FROM salary_items WHERE batch_id = ? AND employee_user_id = ?",
      )
  ...
  private adminIds(batch: BatchRow): string[] {
    return JSON.parse(batch.assigned_admin_ids) as string[];
  }
  ...
  private toBatchSummary(row: BatchRow): SalaryBatchSummary {
    ...
    return {
      ...
      assignedAdminIds: this.adminIds(row),
  ...
  private toItem(row: ItemRow): StoredItem {
    const encrypted: EncryptedPayload = {
      ciphertext: row.fields_ciphertext,
      iv: row.fields_iv,
      authTag: row.fields_auth_tag,
    };
    return {
      id: row.id,
  ...
  private recordEvent(
    table: "salary_deliveries" | "salary_evidence",
    input: any,
  ): any {
    const record = {
      ...input,
    ...
    if (table === "salary_deliveries")
      this.db
        .prepare(
          "INSERT INTO salary_deliveries (id, batch_id, employee_user_id, status, task_id, error, created_at) VALUES (?, ?, ?, ?, ?, ?, ?)",
        )
        .run(
          record.id,
          record.batchId,
      ...
      this.db
        .prepare(
          "INSERT INTO salary_evidence (id, batch_id, employee_user_id, event_type, fingerprint, metadata, created_at) VALUES (?, ?, ?, ?, ?, ?, ?)",
        )
        .run(
          record.id,
          record.batchId,
