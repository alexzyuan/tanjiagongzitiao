...
- 企业管理员与子管理员拥有相同的全局工资业务权限，包括工资表、报表、存证、设置、归档和权限管理；工资表管理员仍仅能操作被授权的批次。子管理员只能从企业通讯录中选择，服务端必须再次验证所选用户存在于通讯录，不能信任前端传入的用户 ID。
- 员工工资条入口仅在移动端钉钉内展示；服务端仍以钉钉免登身份做数据访问控制，不能把客户端显示限制作为授权边界。
- 钉钉工资通知默认使用已验证可投递的工作通知 `link` 消息，包含工资条图片缩略图和明细跳转。显式配置 `DINGTALK_NOTIFICATION_CHANNEL=interactive_card` 时，改用受支持的卡片实例创建/投放 API 和已发布模板；卡片只传递工资期标题与明细 URL，按钮只打开明细，不允许转发，也不配置 Stream 回调。不得在旧工作通知接口上猜测 `action_card` 载荷。
...
当前默认且已验证的工资触达方式是工作通知 `asyncsend_v2` 的 `link` 消息；本项目不使用 DING。互动卡片是显式 opt-in 的独立通道，必须配置已发布卡片模板并通过真实环境验证；它不复用旧接口的 `action_card` 载荷，不配置 Stream 回调，按钮仅跳转工资明细且禁止转发。未完成 OAuth/token/调度链路前，不得启用或声称“未查看/未确认待办”已启用。
